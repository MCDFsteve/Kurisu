...
function generateFFmpegCommand(filePath, userCommand, ffmpegPath) {
    ...
    const message1 = "从现在开始直到对话结束,请将我输入给你的内容处理成可用的ffmpeg命令,";
    ...
    let content;
    if (fileCount > 1) {
        content = `${message1}我会在句首给出多个输入文件的路径（逗号隔开）,输入输出文件路径用双引号括起来，请按顺序生成多个ffmpeg命令并使用 || 符号分隔给出，同时输出的文件名字请继承输入的文件名字。默认的输出文件请放在${outputPath}下。${message2} ${filePath} ${userCommand}`;
    } else {
        content = `${message1}从现在开始直到对话结束,请将我输入给你的内容处理成可用的ffmpeg命令,我会在句首给出输入文件的路径,输入输出文件路径用双引号括起来。默认的输出文件请放在${outputPath}下，、输出文件命名为${getCurrentTimestamp()}（如果在描述里手动指定了新的路径则请使用指定的命名）。${message2} ${filePath} ${userCommand}`;
    }
    const postData = {
        model: "gpt-3.5-turbo",
        temperature: 0.1,
